...
static int pt__lookup (unsigned int page_number)
{
  ...
  //Trouve la page.
  struct page p = page_table[page_number];
  //Retourne le numero de frame si elle est valide.
  if (p.valid) {
    return p.frame_number;
  }
  //Retourne -1 si la page n'est pas valide.
  return -1;
}
...
{
  // TODO: COMPLÉTER CETTE FONCTION.
  //Sauvegarde le numero de frame.
  page_table[page_number].frame_number = frame_number;
  //Rend l'entree valide.
